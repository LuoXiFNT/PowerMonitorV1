...
/**
 * OLED I2C DMA发送队列上下文
 * 包含队列缓存、队头队尾、忙状态以及发送工作缓存
 */
OLED_I2C_Tx_Queue_t oled_i2c_tx_queue = {0};
...
/**
 * 函    数：判断OLED I2C发送是否空闲
 * 参    数：无
 * 返 回 值：1 空闲，0 忙
 */
uint8_t OLED_IsTxIdle(void)
{
  if (oled_i2c_tx_queue.busy != 0U)
  {
    return 0;
  }
  return (oled_i2c_tx_queue.head == oled_i2c_tx_queue.tail) ? 1U : 0U;
}

/**
 * 函    数：压入一个发送节点到DMA队列
 * 参    数：type 节点类型（命令/数据）
 * 参    数：command 命令字节（当type为命令时有效）
 * 参    数：data 数据指针（当type为数据时有效）
 * 参    数：size 数据长度
 * 返 回 值：1 压入成功，0 队列满
 */
uint8_t OLED_I2C_QueuePush(uint8_t type, uint8_t command, const uint8_t *data, uint8_t size)
{
  ...
  __disable_irq();
  head = oled_i2c_tx_queue.head;
  next = (uint8_t)((head + 1U) % OLED_I2C_TX_QUEUE_LEN);
  if (next == oled_i2c_tx_queue.tail)
  {
    __enable_irq();
    return 0;
  }

  oled_i2c_tx_queue.queue[head].type = type;
  oled_i2c_tx_queue.queue[head].cmd = command;
  oled_i2c_tx_queue.queue[head].size = size;
  oled_i2c_tx_queue.queue[head].data_ptr = data;
  oled_i2c_tx_queue.head = next;
  __enable_irq();

  OLED_I2C_TryStartNext();
  return 1;
}
...
{
  if (oled_i2c_tx_queue.head != oled_i2c_tx_queue.tail)
  {
    oled_i2c_tx_queue.tail = (uint8_t)((oled_i2c_tx_queue.tail + 1U) % OLED_I2C_TX_QUEUE_LEN);
  }
}
...
{
  uint8_t tx_size;
  const OLED_I2C_TxNode_t *node;

  if (oled_i2c_tx_queue.busy != 0U ||
      oled_i2c_tx_queue.head == oled_i2c_tx_queue.tail ||
      HAL_I2C_GetState(&OLED_I2C) != HAL_I2C_STATE_READY)
  {
    return;
  }

  node = &oled_i2c_tx_queue.queue[oled_i2c_tx_queue.tail];
  if (node->type == OLED_I2C_TX_TYPE_CMD)
  {
    oled_i2c_tx_queue.work_buf[0] = 0x00;
    oled_i2c_tx_queue.work_buf[1] = node->cmd;
    tx_size = 2U;
  }
  else
  {
    oled_i2c_tx_queue.work_buf[0] = 0x40;
    memcpy(&oled_i2c_tx_queue.work_buf[1], node->data_ptr, node->size);
    tx_size = (uint8_t)(node->size + 1U);
  }

  if (HAL_I2C_Master_Transmit_DMA(&OLED_I2C,
                                  OLED_ADDRESS,
                                  oled_i2c_tx_queue.work_buf,
                                  tx_size) == HAL_OK)
  {
    oled_i2c_tx_queue.busy = 1U;
  }
}
...
{
  OLED_I2C_QueuePopCurrent();
  oled_i2c_tx_queue.busy = 0U;
  OLED_I2C_TryStartNext();
}
...
{
  OLED_I2C_QueuePush(OLED_I2C_TX_TYPE_CMD, Command, NULL, 0U);
}
...
{
  if ((Data == NULL) || (Count == 0U) || (Count > 128U))
  {
    return;
  }
  OLED_I2C_QueuePush(OLED_I2C_TX_TYPE_DATA, 0U, Data, Count);
}
...
void OLED_Init(void)
{
  HAL_Delay(100); // 在初始化前，加入适量延时，待OLED供电稳定
